...
declare global {
  interface Window {
    initPlacesAutocompleteAgentEdit?: () => void
  }
}
...
export default function EditListingPage() {
  ...
  useEffect(() => {
    propertyService.getById(id).then((property) => {
      if (!property) { router.push('/agent/properties'); return }
      ...
      setLoading(false)
    }).catch(() => router.push('/agent/properties'))
  }, [id, router])
  ...
  useEffect(() => {
    ...
    window.initPlacesAutocompleteAgentEdit = initAutocomplete
    const script = document.createElement('script')
    script.id = scriptId
    script.src = `https://maps.googleapis.com/maps/api/js?key=${apiKey}&libraries=places&callback=initPlacesAutocompleteAgentEdit`
    script.async = true
            ...
            {AMENITIES.map((amenity) => {
              ...
              return (
                <button key={amenity} type="button" onClick={() => toggleAmenity(amenity)}
                  className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition-colors ${checked ? 'border-blue-500 bg-violet-500/10 text-blue-300' : 'border-slate-700 bg-slate-900/50 text-slate-400 hover:border-slate-600 hover:text-slate-300'}`}>
